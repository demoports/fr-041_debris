...
const scalarZeroPowCell = D.Bitmap_Cell(
  3, 3, 0xffffffff, 0xff000000, 0xff000000, 1, 1, 0, 0, 0, 0, 0, 0,
);
for (let offset = 0; offset < scalarZeroPowCell.data.length; offset += 4) {
  assert.deepEqual(Array.from(scalarZeroPowCell.data.subarray(offset, offset + 4)),
    [0, 0, 0, 0x7ffe], 'scalar Cell keeps native sFPow(0, 0) == 0');
}
...
//
// Generated by tools/oracles/bitmap_cell_oracle.cpp and checked byte for byte
// against the released routines under their 0x103f x87 control word. Op 452
// catches the PC=24 result stored after FSQRT; ops 475 and 481 catch sFPow's
// unusual zero-base shortcut (0^0 is zero, not one).
const savedTextureOffset = D.bitmapTextureSizeOffset;
const productionCellCases = [
  {
    operationId: 452, textureOffset: -2, dimensions: [128, 128], hash: 3896204010,
    sha256: '80e5c9203b669b088a63529f27d126885aab0144a2ee1689e9fcf3e2ffe4de73',
  },
  {
    operationId: 475, textureOffset: -1, dimensions: [256, 256], hash: 1660067269,
    sha256: '09dbd3f89460cca8f244d1894695f52df7073f5de4aebc07f4ebddcef1388c48',
  },
  {
    operationId: 481, textureOffset: 0, dimensions: [512, 512], hash: 141860293,
    sha256: '5b2f2ecd09d0ea97a02ae7e5ff2172261a300589590d6bd22956f9426cc2062d',
  },
];
try {
